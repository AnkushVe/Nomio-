...
interface NavbarProps {
  user?: User | null;
  currentPage?: 'home' | 'map-lab' | 'destinations';
  onNavigate?: (page: 'home' | 'map-lab' | 'destinations') => void;
}

const Navbar: React.FC<NavbarProps> = ({ user, currentPage, onNavigate }) => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const handleNavigation = (page: 'home' | 'map-lab' | 'destinations') => {
    if (onNavigate) {
      onNavigate(page);
    }
    setIsMenuOpen(false);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-24">
          ...
          <div className="hidden md:flex items-center space-x-10">
            ...
            <button 
              onClick={() => handleNavigation('map-lab')}
              className={`transition-colors font-semibold ${
                currentPage === 'map-lab' 
                  ? 'text-blue-600' 
                  : 'text-gray-800 hover:text-blue-600'
